...
class MainWindow(Window):
    ...
    def _task_register(self, username, password):
        try:
            ...
            if(response['code'] != Code.SUCCESS):
                ...
                showInfo('Successfully registered! Now please login.')
        except Exception, e:
            traceback.print_exc(e)
            showError('Connection failed. Check your network and retry.')
        finally:
            client.shutdown()
    ...
    def register(self):
        ...
        if len(username) == 0:
            ...
            Thread(self._task_register(username, password)).start()
    ...
    def _task_login(self, username, password):
        try:
            ...
            if(response['code'] != Code.SUCCESS):
                ...
                self._app.changeWindow(ChatWindow(self._app))
        except Exception, e:
            traceback.print_exc(e)
            showError('Connection failed. Check your network and retry.')
            self._app.shutdownClient()
    ...
    def login(self):
        ...
        if len(username) == 0:
            ...
            Thread(self._task_login(username, password)).start() 
...
class ChatWindow(Window):
    def createWidgets(self):
        ...
        client.onServerCloseConnection(self.onDisconnected)
    ...
    def createHallMenuBar(self):
        ...
        if room is None:
            ...
            self._button_channel['command'] = lambda: self.doEnterRoom(room)
    ...
    def sendBroadcast(self):
        ...
        Thread(self._task_sendBroadcast(message)).start()
    ...
    def _task_createRoom(self, config):
        try:
            ...
            if(response['code'] != Code.SUCCESS):
                ...
                self._app.getUser().setRoom(room)
                self.doEnterRoom(room)
        except Exception, e:
            traceback.print_exc(e)
            showError(str(e))
    ...
    def createRoom(self):
        ...
        if dialog.isOK():
            ...
            Thread(self._task_createRoom(config)).start()
    ...
    def enterRoom(self):
        ...
        if dialog.isOK():
            ...
            Thread(self._task_enterRoom(roomname)).start()
    ...
    def exitRoom(self):
        ...
        self.doEnterHall()
        Thread(self._task_exitRoom()).start()
    ...
    def doEnterRoom(self, room):
        if(self._frame_top_menu):
            self._frame_top_menu.destroy()
        if(self._frame_main):
            self._frame_main.destroy()
        self._frame_main = RoomFrame(self, room)
        self._frame_main.grid(row=2)
    ...
    def sendRoomMessage(self):
        ...
        username = self._app.getUser().getName()
        self._frame_main.insertMessage(username, message)
        Thread(self._task_sendRoomMessage(message)).start()
    ...
    def onDisconnected(self):
        self.logout()
        showError('Connection wad broken. Check your network.')
...
class EnterRoomDialog(MyDialog):
    def body(self, master):
        ...
        self._roomname = None
        Thread(self._task_getRooms()).start()
